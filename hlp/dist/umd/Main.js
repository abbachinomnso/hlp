...
function tokFunction(stream, state) {
    ...
    var activePatternResult394 = _EatSpace___(stream);
    ...
    if (activePatternResult394 != null) {
        ...
        var activePatternResult393 = function () {
        ...
        if (activePatternResult393 != null) {
            ...
            var activePatternResult390 = function () {
                var regExpStr_3 = "^//";
            ...
            if (activePatternResult390 != null) {
                var activePatternResult391 = _EatLine___(activePatternResult390);

                if (activePatternResult391 != null) {
                    $var1 = [0, activePatternResult391];
                } else {
                    $var1 = [1];
                }
            } else {
            ...
            switch ($var1[0]) {
                ...
                case 1:
                    var activePatternResult388 = function () {
                    ...
                    if (activePatternResult388 != null) {
                        ...
                        var activePatternResult386 = function () {
                            var regExpStr_2 = "^.";
...
function readFileSimple(fName) {
    try {
        return electronFS.readFileSync(fName, "utf8");
    } catch (matchValue) {
        return null;
    }
}

function readFromFileDialog(fs) {
    var chooseFileOptions = {
        properties: ["openFile"]
    };
    var optFName = void 0;
    var matchValue = dialog.showOpenDialog(chooseFileOptions);

    if (isArray(matchValue)) {
        optFName = matchValue[0];
    } else {
        optFName = null;
    }

    if (optFName != null) {
        var matchValue_1 = readFileSimple(optFName);

        if (matchValue_1 != null) {
            return [optFName, matchValue_1];
        } else {
            return null;
        }
    } else {
...
function render() {
    var showMessage = function showMessage(s) {
        var showMessageOptions = function showMessageOptions(s_1) {
            return {
                buttons: ["OK", "Terrible"],
                title: "My Box",
                message: s_1,
        ...
        var ret = dialog.showMessageBox(showMessageOptions(s));
        fsFormat("Messagebox returned %f")(function (x) {
            console.log(x);
        })(ret);
    };

    var matchValue = readFromFileDialog(null);

    if (matchValue == null) {
        fsFormat("No file name given!")(function (x) {
            console.log(x);
        });
    } else {
        var fName = matchValue[0];
        var data = matchValue[1];
        fsFormat("File %s has size %d bytes")(function (x) {
            console.log(x);
        })(fName)(data.length);
        showMessage(fsFormat("File %s has length %d")(function (x) {
            return x;
        })(fName)(data.length));
    }
}
...
function main() {
    ...
    var editId = getById("code");
    fsFormat("Creating editor")(function (x) {
        console.log(x);
    });
    var initOptions$$1 = {
        lineNumbers: true,
    ...
    var cmEditor = CodeMirror.fromTextArea(editId, initOptions$$1);
    fsFormat("Setting editor value")(function (x) {
        console.log(x);
    });
    cmEditor.setValue("Enter Code hello");
    fsFormat("Line tokens: %A")(function (x) {
        console.log(x);
    })(cmEditor.getLineTokens(0));
    var test = getById("test");
    test.defaultValue = "Printing value here";
    var runButton = getById("compile");
    runButton.textContent = "Compile";
    runButton.addEventListener('click', function (_arg1) {
        getText(cmEditor.getValue());
        return null;
    });
    render();
    fsFormat("Main code finished")(function (x) {
        console.log(x);
    });
}
main();
